...
class Eq3Thermostat extends utils.Adapter {
    ...
    async onReady() {
        ...
        if (this.config.getEQ3Devices.length) {
            for (let nDev = 0; nDev < this.config.getEQ3Devices.length; nDev++) {
                ...
                await this.setObjectNotExists(sDevMAC+'.target_temperature', { type: 'state', common: { name: 'target_temperature', role: 'level.temperature', read: true, write: true, type: 'number', unit: '°C', min: 5, max: 30 }, native: {} });
                await this.setObjectNotExists(sDevMAC+'.actual_temperature', { type: 'state', common: { name: 'actual_temperature', role: 'value.temperature', read: true, write: false, type: 'number', unit: '°C', min: 5, max: 30 }, native: {} });
                ...
                await this.setObjectNotExists(sDevMAC+'.force_temperatur', { type: 'state', common: { name: 'force_temperatur', role: 'switch', read: true, write: true, type: 'boolean' }, native: {} });
            }
        }
    ...
    onStateChange(id, state) {
        if (state) {
            ...
            if (state.from !== 'system.adapter.' + this.namespace) {
                ...
                if (stateName === 'target_temperature') {
    ...
    onMessage(obj) {
        ...
        if (typeof obj === 'object') {
            ...
            if (obj.command === 'findDevices') {
                //TT, save Command Result true/false
                try {
                    stdout = execSync('timeout -s INT 8s stdbuf -oL hcitool lescan').toString().replace(new RegExp('\r?\n','g'), '<br>');
                } catch (e) {
                    stdout = 'Error: ' + e.stdout.toString().replace(new RegExp('\r?\n', 'g'), '<br>');
                }
    ...
    async fUpdateDevObj(aDevValues, sDevMAC, sDevName) {
        //0 = Temperature | 1 = Valve | 2 = LowBattaryAlarm | 3 = NoConnection | 4 = Boost
        let actualTemp = aDevValues[0];
        if (await this.getStateAsync(sDevMAC + '.force_temperatur')) {
            actualTemp = await this.getStateAsync(sDevMAC + '.target_temperature');
        }
        this.setStateAsync(sDevMAC + '.actual_temperature', { val: actualTemp, ack: true });
